...
class SoftMax(Activation):
    ...
    def forward(self, Z: np.ndarray) -> np.ndarray:
        ...
        ### YOUR CODE HERE ###
        M = np.max(Z, axis=1)
        EXPS = np.exp(np.transpose(Z.T-M))
        return np.transpose(EXPS.T/(np.sum(EXPS, axis=1)))
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        for i in range(batch_size):
            diagonal = np.diag(gZ[i, :])
            grad = diagonal - np.outer(gZ[i, :], gZ[i, :])
            grads.append(dY[i, :] @ grad)
